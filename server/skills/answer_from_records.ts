...
"citedSlipIds" must list the "id" field of every record you actually used to answer. If the records don't contain enough information to answer, say so plainly in "answer" and leave "citedSlipIds" empty — never fabricate an answer from outside knowledge.`;

export function buildQueryPrompt(question: string, records: Record<string, unknown>[]): string {
  return [
    "You are answering a question about raw-material intake records for a business.",
    ...
    OUTPUT_SCHEMA_HINT,
    "",
    "Records (JSON array, each has an \"id\" field to cite):",
...
export function parseQueryResponse(raw: string): QueryAnswer {
  ...
  if (typeof parsed.answer !== "string" || !Array.isArray(parsed.citedSlipIds)) {
    throw new Error("Model response did not match the expected { answer, citedSlipIds } shape");
  }

  return { answer: parsed.answer, citedSlipIds: parsed.citedSlipIds };
}
